...
**Document Version:** 0.9 (Phase 2 atomic-checkout + real-discount-integration + idempotency-middleware + checkout-rate-limiter sub-deliverables delivered 2026-07-18: 38 tests passing in `Basket.API.Tests` — 9 Phase 1 + 3 atomic-checkout + 11 real-discount + 8 idempotency + 7 rate-limiter. 0 build errors, 0 new warnings. Strict build `-p:TreatWarningsAsErrors=true` clean. Drift items captured inline in §6 Phase 2. Remaining Phase 2 work: `PaymentMethod` enum + `PaymentMethodSummary` event payload (2.1 BuildingBlocks contribution), wrapper-record cleanup + 201-vs-200 on PUT (2.5), the §0.4.10 spoofing-footgun validator fix.)
**Last Updated:** 2026-07-18
**Maintained By:** Basket working group (TBD)
**Status:** ✅ Phase 1 (2026-07-17) + Phase 2 atomic-checkout + real-discount-integration + idempotency-middleware + checkout-rate-limiter (2026-07-18) delivered. ⏳ Phase 2 remaining work (payment-method redaction, wrapper-record cleanup). ⏳ Phase 3 (cache stampede protection + expiry sweep + DELETE 204 + URL cleanup). ⏳ Phase 4 (gRPC resilience + observability + Swagger). ⏳ Phase 5 (test project expansion + Testcontainers).
