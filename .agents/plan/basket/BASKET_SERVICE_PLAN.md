...
**Document Version:** 0.8 (Phase 2 atomic-checkout + real-discount-integration + idempotency-middleware sub-deliverables delivered 2026-07-18: 31 tests passing in `Basket.API.Tests` — 9 Phase 1 + 3 atomic-checkout + 11 real-discount + 8 idempotency. 0 build errors, 0 new warnings. Strict build `-p:TreatWarningsAsErrors=true` clean. Drift items captured inline in §6 Phase 2. Remaining Phase 2 work: `PaymentMethod` enum + `PaymentMethodSummary` event payload (2.1 BuildingBlocks contribution), `CheckoutCartRateLimiter` (2.4), wrapper-record cleanup + 201-vs-200 on PUT (2.5), the §0.4.10 spoofing-footgun validator fix.)
**Last Updated:** 2026-07-18
**Maintained By:** Basket working group (TBD)
**Status:** ✅ Phase 1 (2026-07-17) + Phase 2 atomic-checkout + Phase 2 real-discount-integration + Phase 2 idempotency-middleware (2026-07-18) delivered. ⏳ Phase 2 remaining work (payment-method redaction, rate limiter, wrapper-record cleanup). ⏳ Phase 3 (cache stampede protection + expiry sweep + DELETE 204 + URL cleanup). ⏳ Phase 4 (gRPC resilience + observability + Swagger). ⏳ Phase 5 (test project expansion + Testcontainers).
