...
7. **`xunit.runner.json` parallelism quirk (NOT fixed).** Concurrent `BasketCacheLockRegistryTests` report "test host blocked" when 4+ cache-lock tests run in the same session; each test passes in isolation. The `xunit.parallelizeTestCollections=false` fix is deferred to Phase 5.1 alongside other test-infrastructure work.
...
- [x] **Phase 5** ✅ **Phase 5.0 partial delivery 2026-07-26 + 2026-07-27** — `Basket.API.Tests` project expansion (xUnit + FluentAssertions + NSubstitute + Testcontainers + Verify; project shell scaffolded in Phase 1 with 9 unit tests) + `BasketWebApplicationFactory` (Testcontainers Postgres + Redis + RabbitMQ; `TestAuthHandler` mirrors Discount's pattern — deliberate deviation from the plan's literal `JwtTestAuthenticationHandler` shape) + endpoint contract tests for every (verb, status) combo (`GetCart` 4/4 + `DeleteCart` 3/3 + `UpsertCart` 6/6 + `CheckoutCart` 8/8) + production fixes (`ICurrentRestaurantProvider` registration in `Program.cs`, `Marten.NodaTime 8.37.4` + `UseNodaTime()`, `Basket.API.Tests/Integration/NodaTimeJson.cs` for the UpsertCart body-binding fix, `StubOkResult` test double for the CheckoutCart `response has already started` fix). **132 / 132 tests pass** in `Basket.API.Tests` (111 unit + 21 integration). **Phase 5.1 follow-ups (still in scope of this plan, scheduled as the next PR):** (1) `OpenApiGenerationTests.AllEndpointsDocumented` — parses `/swagger/v1/swagger.json`, asserts every `MapBasketGroup` endpoint is documented + no `MapBasketGroup` endpoint is undocumented; (2) first commit of `docs/api/basket-api-v1.json`; (3) `BasketExpirySweepTests.ExpiredBasket_Deleted` + `.LiveBasket_NotTouched` against the Postgres fixture + `TestClock : IClock` + `BasketExpirySweepWebApplicationFactory` re-enabling the sweep service; (4) `BasketSnapshots.VerifyAllEndpoints` Verify snapshot suite under `Services/Basket/Basket.API.Tests/Snapshots/`; (5) `.github/workflows/basket-tests.yml` CI gate; (6) `current-architecture.md` §11/§12 updates; (7) `xunit.runner.json` `xunit.parallelizeTestCollections=false` fix. See §6 Phase 5 status header for the full delivery log + the seven inline drift items.
...
**Document Version:** 1.2 (Phase 5.0 partial delivery 2026-07-26 + 2026-07-27 — Testcontainers + WebApplicationFactory integration test scaffolding + endpoint contract tests for `GetCart` 4/4 + `DeleteCart` 3/3 + `UpsertCart` 6/6 + `CheckoutCart` 8/8; production fixes for `ICurrentRestaurantProvider` registration, `Marten.NodaTime` plugin, `NodaTimeJson` test helper, `StubOkResult` test double. BuildingBlocks + Basket build clean with `-p:TreatWarningsAsErrors=true`. **132 / 132** tests passing in `Basket.API.Tests` (111 unit + 21 integration). The Basket API surface is feature-complete and the test infrastructure is operational. **Remaining Phase 5.1 follow-ups** (all in scope of this plan, scheduled as the next PR): OpenAPI mirror test + first commit of `docs/api/basket-api-v1.json` + `BasketExpirySweepTests` Marten fan-out assertions + Verify snapshot suite + `.github/workflows/basket-tests.yml` + `current-architecture.md` §11/§12 updates + `xunit.runner.json` parallelism fix. **Phase 5.x deferred** (still in scope, future PR): outbox multi-replica `FOR UPDATE SKIP LOCKED` switch + `ISessionFactory` ambient tenant wiring + multi-tenancy strategy resolution.)
