...
- [ ] **Real-discount integration (sub-deliverable 2.2).** `StoreBasket` discount `#warning` removed; checkout publish-and-delete becomes one transaction.
...
- [ ] **Phase 2** — 🚧 **Atomic-checkout sub-deliverable delivered 2026-07-18** (commit pending; 12 tests passing in `Basket.API.Tests`, 0 build errors, 0 new warnings). New `CheckoutBasketOutboxMessage` Marten document (`MultiTenanted()` + `[DuplicateField]` on `OccurredOn`/`DispatchedAt`); new `CheckoutBasketOutboxDispatcher : BackgroundService` (Marten-flavored — does NOT extend `OutboxDispatcher<TContext>`, drift item 1; linear, no `BrokerHealthState` yet, drift item 2; Marten LINQ + optimistic concurrency, not raw `FOR UPDATE SKIP LOCKED`, drift item 3); `CheckoutBasketCommandHandler` rewritten to atomic publish-and-delete (one `SaveChangesAsync` covers outbox stage + basket delete + NodaTime-correct cache invalidation via new `IBasketRepository.InvalidateCacheAsync`); `Outbox` section in `appsettings.json`; 3 new unit tests (empty-basket skip / atomic one-commit / payload round-trip). Doc updates: `current-architecture.md` §4.3 + §5.2 + §6 + §9. Still ⏳: real-discount integration (`#warning` TODO removal — sub-deliverable 2.2), idempotency middleware (2.3), `PaymentMethod` enum + `PaymentMethodSummary` event payload (2.1 BuildingBlocks contribution), `CheckoutCartRateLimiter` (2.4), wrapper-record cleanup + 201-vs-200 on PUT (2.5), the §0.4.10 spoofing-footgun validator fix.
...
**Document Version:** 0.6 (Phase 2 atomic-checkout sub-deliverable delivered 2026-07-18: 12 tests passing in `Basket.API.Tests` — 9 Phase 1 + 3 new. 0 build errors, 0 new warnings. Drift items 1–6 captured inline in §6 Phase 2. Remaining Phase 2 work: real-discount integration, idempotency middleware, `PaymentMethod` enum + `PaymentMethodSummary` event payload, `CheckoutCartRateLimiter`, wrapper-record cleanup + 201-vs-200 on PUT, the §0.4.10 spoofing-footgun validator fix.)
**Last Updated:** 2026-07-18
**Maintained By:** Basket working group (TBD)
**Status:** ✅ Phase 1 (2026-07-17) + Phase 2 atomic-checkout sub-deliverable (2026-07-18) delivered. ⏳ Phase 2 remaining work (real-discount integration, idempotency middleware, payment-method redaction, rate limiter). ⏳ Phase 3 (cache stampede protection + expiry sweep + DELETE 204 + URL cleanup). ⏳ Phase 4 (gRPC resilience + observability + Swagger). ⏳ Phase 5 (test project expansion + Testcontainers).
