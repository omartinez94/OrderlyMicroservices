# Basket.API — Service Plan (v1)

> **Scope:** completion plan for the existing `Basket.API` microservice. Closes the gaps surfaced by a csharp-expert review of `Services/Basket/Basket.API/` against `docs/architecture/architecture.md`, `docs/architecture/current-architecture.md`, and the rest of the solution. Today the four happy paths (`GET / PUT / DELETE / POST checkout`) are wired through Carter → MediatR → Marten → Redis → MassTransit, but the service is **not production-ready**: tenant safety is absent, authorization does not bind the authenticated user to the basket id, discount integration is a `#warning` no-op, checkout publish-then-delete is non-atomic, and there is **no test project**. This is an *evolution* plan, not a green-field design — all four endpoints exist; the work is hardening and adding the missing cross-cutting layers.
>
...
- `CheckoutCartHandler.Handle` becomes:
  1. `IBasketIdentityGuard` (§0.4.9) throws `ForbiddenException` if the JWT subject or restaurant claim doesn't match.
  2. Load basket.
  3. **Validate basket is non-empty + has discount snapshot + `ExpiresAt > now`.**
  4. Build `BasketCheckoutEvent` with **redacted payment fields** (`PaymentMethod` enum + last-four + brand only).
  5. Insert outbox row.
  6. Delete basket (or `MarkDeleted`) in the same session.
  7. Commit.
- New `IdempotencyMiddleware` (Carter `IEndpointFilter` on `POST /cart/checkout`):
...
- New `PaymentMethod` enum (see §0.4.10) replacing the `int` field; `BasketCheckoutDto.CardNumber` stays as `string` but the new `BasketCheckoutEvent` payload includes **only** `PaymentMethodSummary { Method, Brand, LastFour }`.
- `MapBasketGroup` adds `RequireRateLimiting("checkout")` (§0.4.8) on the checkout endpoint only.
- The wrapper records `CheckoutBasketRequest(BasketCheckoutDto)` and `StoreBasketRequest(Basket)` are deleted; the command record binds to the request body directly (api-design-principles Pattern 1).
- PUT endpoint returns **201 Created** + `Location: /api/v1/cart` when the cart is new and **200 OK** when it already existed; the handler exposes an `UpsertCartResult { bool IsCreated }` (see §0.4.3).
- Tests:
  ...
  - `CheckoutCartHandlerTests.Success_PublishesAndDeletes_Atomic`.
  - `CheckoutCartHandlerTests.PublishFails_BasketNotDeleted`.
...
- **Doc-update scope:** §4.3 (atomic checkout, outbox dispatch, idempotency envelope, rate limiter), §5.2 (`BasketCheckoutEvent` SchemaVersion + card-redaction note), §9 (outbox dispatcher entry), §11 (idempotency-key config), §12 (OpenTelemetry for outbox).
