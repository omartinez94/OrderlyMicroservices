...
- [ ] **Idempotency middleware (sub-deliverable 2.3).** Carter `IEndpointFilter` on `POST /cart/checkout`:
  - Reads `Idempotency-Key` header (UUID v4; rejected if not).
  - Looks up `basket:idem:{userId}:{restaurantId}:{key}` in Redis; on hit, returns the cached `200 OK + body`.
  - On miss, locks via `SETNX` + 24h TTL, runs the pipeline, caches the response.
  - **Key reuse with a different request body → `422 Unprocessable Content`** (per IETF `draft-ietf-httpapi-idempotency-key-header`, **not 409** — the plan's earlier 409 used the wrong code for the wrong reason).
  - HMAC envelope: `HMAC-SHA256(serverSecret, userId + restaurantId + sha256(requestBody))` — same shape as DISCOUNT_SERVICE_PLAN §0.4.1; the secret comes from `appsettings.json` (`Basket:IdempotencyKey`) and is never logged.
- [ ] **Payment-method redaction (sub-deliverable 2.1, BuildingBlocks contribution).** New `PaymentMethod` enum (see §0.4.10) replacing the `int` field; `BasketCheckoutDto.CardNumber` stays as `string` but the new `BasketCheckoutEvent` payload includes **only** `PaymentMethodSummary { Method, Brand, LastFour }`. The `BasketCheckoutEvent` shape change is a BuildingBlocks contribution (the event lives in `BuildingBlocks.Messaging.Events`); Ordering's consumer must be updated in lockstep — flagged in §7 cross-service notes.
- [ ] **Checkout rate-limiter (sub-deliverable 2.4).** `MapBasketGroup` adds `RequireRateLimiting("checkout")` (§0.4.8) on the checkout endpoint only — `(userId, restaurantId)` keyed, 5/minute, `Retry-After` header on 429.
- [ ] **Wrapper-record cleanup (sub-deliverable 2.5).** The wrapper records `CheckoutBasketRequest(BasketCheckoutDto)` and `StoreBasketRequest(Basket)` are deleted; the command record binds to the request body directly (api-design-principles Pattern 1). PUT endpoint returns **201 Created** + `Location: /api/v1/cart` when the cart is new and **200 OK** when it already existed; the handler exposes an `UpsertCartResult { bool IsCreated }` (see §0.4.3). The §0.4.10 spoofing-footgun fix (`RuleFor(x => x.UserId).Equal(Guid.Empty)` → 422 when body carries a value) lands here.
- [ ] **Idempotency middleware (sub-deliverable 2.3).** Carter `IEndpointFilter` on `POST /cart/checkout`:
  - Reads `Idempotency-Key` header (UUID v4; rejected if not).
  - Looks up `basket:idem:{userId}:{restaurantId}:{key}` in Redis; on hit, returns the cached `200 OK + body`.
  - On miss, locks via `SETNX` + 24h TTL, runs the pipeline, caches the response.
...
- [ ] **Phase 2** — 🚧 **Atomic-checkout + real-discount integration sub-deliverables delivered 2026-07-18** (commit pending; 23 tests passing in `Basket.API.Tests`, strict build `-p:TreatWarningsAsErrors=true` clean). Atomic-checkout: new `CheckoutBasketOutboxMessage` Marten document (`MultiTenanted()` + `[DuplicateField]` on `OccurredOn`/`DispatchedAt`); new `CheckoutBasketOutboxDispatcher : BackgroundService` (Marten-flavored — does NOT extend `OutboxDispatcher<TContext>`, drift item 1; linear, no `BrokerHealthState` yet, drift item 2; Marten LINQ + optimistic concurrency, not raw `FOR UPDATE SKIP LOCKED`, drift item 3); `CheckoutBasketCommandHandler` rewritten to atomic publish-and-delete (one `SaveChangesAsync` covers outbox stage + basket delete + NodaTime-correct cache invalidation via new `IBasketRepository.InvalidateCacheAsync`); 3 new unit tests. Real-discount integration (sub-deliverable 2.2): new `Basket.API.Discount.IDiscountLookup` abstraction + `GrpcDiscountLookup` (raw gRPC client wrapped so the discount loop is unit-testable — gRPC's `AsyncUnaryCall<T>` doesn't mock cleanly); `Basket.DiscountAmount` (clamped to `Subtotal`) + `Basket.AppliedCoupons: List<CouponSnapshot>` + `Basket.Total` derived; `StoreBasketHandler` rewritten with `Parallel.ForEachAsync(MaxDegreeOfParallelism = 4)` polyfill and fail-closed gRPC policy; `CachedBasketRepository` NodaTime round-trip fix (Phase 1 drift item); 11 new unit tests. Doc updates: `current-architecture.md` §4.3 + §5.2 + §6 + §9. Still ⏳: idempotency middleware (2.3), `PaymentMethod` enum + `PaymentMethodSummary` event payload (2.1 BuildingBlocks contribution), `CheckoutCartRateLimiter` (2.4), wrapper-record cleanup + 201-vs-200 on PUT (2.5), the §0.4.10 spoofing-footgun validator fix.
...
**Document Version:** 0.7 (Phase 2 atomic-checkout + real-discount-integration sub-deliverables delivered 2026-07-18: 23 tests passing in `Basket.API.Tests` — 9 Phase 1 + 3 atomic-checkout + 11 real-discount. 0 build errors, 0 new warnings. Strict build `-p:TreatWarningsAsErrors=true` clean — the `#warning` TODO blocker is gone. Drift items captured inline in §6 Phase 2. Remaining Phase 2 work: idempotency middleware (2.3), `PaymentMethod` enum + `PaymentMethodSummary` event payload (2.1 BuildingBlocks contribution), `CheckoutCartRateLimiter` (2.4), wrapper-record cleanup + 201-vs-200 on PUT (2.5), the §0.4.10 spoofing-footgun validator fix.)
**Last Updated:** 2026-07-18
**Maintained By:** Basket working group (TBD)
**Status:** ✅ Phase 1 (2026-07-17) + Phase 2 atomic-checkout + Phase 2 real-discount-integration (2026-07-18) delivered. ⏳ Phase 2 remaining work (idempotency middleware, payment-method redaction, rate limiter, wrapper-record cleanup). ⏳ Phase 3 (cache stampede protection + expiry sweep + DELETE 204 + URL cleanup). ⏳ Phase 4 (gRPC resilience + observability + Swagger). ⏳ Phase 5 (test project expansion + Testcontainers).
