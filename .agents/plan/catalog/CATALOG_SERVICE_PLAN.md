...
> **Status: ✅ COMPLETED — 2026-07-18** (Document Version `2.11`)
>
> Phases 1–5 + 6.2 + Cleanup + Docs all shipped (Document Version 2.10 / last updated 2026-07-13). The two out-of-plan tracking rows (Phase 6.0 Reservation/WalkInQueue → Ordering; Phase 6.1 CustomerFeedback/NotificationLog → Notification) were *not* done by Catalog — they're relocated to sibling plans: Phase 6.0 work is owned by the new `.agents/plan/ordering/RESERVATION_PLAN.md` (Migration-with-Booking-API scope: move Reservation + WalkInQueue + Tables + MergedTables; unified Booking service API); Phase 6.1 work is owned by the existing `.agents/plan/notification/NOTIFICATION_SERVICE_PLAN.md`. BulkOrderUpload stays in Catalog per the original §7.6.0 decision. The plan is closed; new Catalog work belongs in a new plan.
...
> **Out-of-plan entity moves:** Reservation/WalkInQueue → Ordering (now `.agents/plan/ordering/RESERVATION_PLAN.md`), CustomerFeedback/NotificationLog → Notification (`.agents/plan/notification/NOTIFICATION_SERVICE_PLAN.md`).
...
- [x] **Phase 6.0** — *Out-of-plan.* **Relocated to `.agents/plan/ordering/RESERVATION_PLAN.md`** (authored 2026-07-18, scope: Migration-with-Booking-API). Move includes `Reservation` + `WalkInQueue` + `Tables` + `MergedTables` aggregates from Catalog → Ordering; provide a unified Booking service API surface (Carter endpoints consolidating reservation + walk-in + table-assignment flows). BulkOrderUpload stays in Catalog per the original §7.6.0 decision. **When RESERVATION_PLAN ships**, Catalog drops the source aggregates, deletes the model files, removes the endpoints, cancels the Hangfire jobs (re-hosted in Ordering), removes `ReservationReminderDueIntegrationEvent` publishing (re-issued from Ordering), and runs the §6.6 gateway convention + §7.6.2 backfill pattern. Catalog's `Table` / `MergedTable` ownership transfers with the move.
  - [x] **Phase 6.0 doc** — Status block at the top of this file + v2.11 changelog entry note the relocation. `current-architecture.md` refresh lands when RESERVATION_PLAN ships (Catalog §4.2 row updates + new §4.7 Ordering Booking subsection per the plan's §9 doc-update scope).
  - [x] **Phase 6.0 completed** — work is tracked in RESERVATION_PLAN.md. This tracking row stays in the plan as a historical pointer + future-trigger (when the Ordering-side move lands, the Catalog-side source deletion is a mechanical follow-up).
- [x] **Phase 6.1** — *Out-of-plan.* **Remains in `.agents/plan/notification/NOTIFICATION_SERVICE_PLAN.md`** (Document Version 0.1, drafted 2026-07-13). Catalog-side work is `CustomerFeedback` ownership move + `NotificationLog` Marten-document removal per §6.7 merge. **Trigger to act:** when `Services/Notification/Notification.API/` exists *and* its backfill has verified row-parity against `mt_doc_notification_log`.
  - [x] **Phase 6.1 doc** — none required while Catalog waits. When Notification v1 ships, run the §6.7 doc-update scope (Catalog entity table row dropped; new Notification Service section in `current-architecture.md`; mermaid + companion doc cleaned up).
  - [x] **Phase 6.1 completed** — work is tracked in NOTIFICATION_SERVICE_PLAN.md. This tracking row stays in the plan as a historical pointer + future-trigger.
...
**Document Version:** 2.11 (Plan closed 2026-07-18 — Phases 1–5 + 6.2 + Cleanup + Docs shipped; Phase 6.0 + 6.1 tracking rows relocated to sibling plans)
**Last Updated:** 2026-07-18
**Maintained By:** Catalog working group (historical)

> **v2.11 changelog — Plan closed; Phase 6.0 + 6.1 relocated.** The plan is closed. Phases 1–5 + 6.2 + Cleanup + Docs all shipped (Document Version 2.10 / last updated 2026-07-13). The two out-of-plan tracking rows are now owned by sibling plans: **Phase 6.0 → `.agents/plan/ordering/RESERVATION_PLAN.md`** (authored today, scope: Migration-with-Booking-API — move Reservation + WalkInQueue + Tables + MergedTables to Ordering; provide a unified Booking service API). **Phase 6.1 → `.agents/plan/notification/NOTIFICATION_SERVICE_PLAN.md`** (Document Version 0.1, drafted 2026-07-13; Catalog-side work is `CustomerFeedback` ownership move + `NotificationLog` Marten-document removal per §6.7). The Phase 6.0 + 6.1 checklist boxes are ticked (`[x]`) with relocation notes; the work itself is **not** done by Catalog — it lives in the sibling plans. **BulkOrderUpload stays in Catalog** per the original §7.6.0 decision. Status block added at the top of the file. When RESERVATION_PLAN ships, the Catalog-side source deletion is a mechanical follow-up; when NOTIFICATION_SERVICE_PLAN ships, the §6.7 doc-update scope runs.
>
> Document Version bumped `2.10 → 2.11`. No §-level content changes; Status block + Status tracking rows updated.
