...
- [ ] **Phase 1B.c (Discount.Grpc.Tests sibling project; commit C)** — `Discount.Grpc.Tests` (`Microsoft.NET.Sdk`, `IsPackable=false`, `IsTestProject=true`), per-fixture temp-file SQLite, xUnit + FluentAssertions + NSubstitute + `TestTimeProvider` + `TestAuthHandler` mirroring Catalog. ~34 tests across 8 classes: `DiscountPermissionsTests` (`JwtClaimShapeProbe` — 12 policies × 2 shapes × 2 outcomes), `GlobalTenantFilterTests` (cross-tenant Read / Create / Delete), `DiscountExpirySweepTests` (`TestTimeProvider`-pinned sweep + Disabled-flag path + actor-string assertion), `RedeemDiscountRaceTests` (sequential happy + 5-vs-3 concurrent contention), `DiscountOutboxCircuitBreakerTests` (3-failure trip + recovery reset + per-row-no-trip), `DiscountOutboxDeadLetterTests` (future-version quarantined + valid v1 relayed), `RpcEndpointTests` (first gRPC integration tests in the repo: 5 RPCs × happy + 3 negative-path cases), `HealthCheckSplitTests` (`/live` always 200; `/ready` flips 200 ⇄ 503 with `BrokerHealthState`).
  - [ ] **Phase 1B.c doc** — `current-architecture.md` §10 Testing row + `DISCOUNT_SERVICE_PLAN.md` §9 row tick.
  - [ ] **Phase 1B.c completed** — code + doc + plan-update commit landed; Document Version `1.7 → 1.8`. See v1.8 changelog tail.
