...
> **Status: ✅ CLOSED 2026-07-30** (Document Version `2.6`)
>
> All 8 phases + the Phase 8 single follow-up commit landed 2026-07-30. Discount is production-shaped: Coupon + RewardCode + DiscountRule aggregates with full gRPC CRUD, JWT auth + multi-tenancy + transactional outbox + expiry sweep, entity-history publishing on the bus, and the Phase 8 apply-surface (Basket preview-time deduction via `ApplyDiscountsHelper` + Ordering-driven `OrderCreatedConsumer` stub). Unit + integration + consumer test suites green.
>
> **Verification date:** 2026-07-30. Build clean (0 errors, 0 warnings). 10/10 new `ApplyDiscountsHelperTests` pass. All 5 service hosts + 2 consumer implementations compile end-to-end.
>
> **Phase 8 implementation notes (2026-07-30):**
> 1. **Proto `RedeemDiscountRequest.order_id`** — added field 3 (optional string). Manual callers (admin UI / preview path) leave it empty; the `OrderCreatedConsumer` stub populates it from `event.OrderId`. Empty → publish site defaults to `Guid.Empty` per the v2.6 implementation notes.
> 2. **`DiscountAppliedIntegrationEvent` v2** — added `OrderId` (`Guid`) + `AppliedAt` (`Instant`). Base-class `IntegrationEvent.MessageVersion` bumps to `2`; outbox row's `SchemaVersion` column mirrors the same value per the `MessageVersion ↔ SchemaVersion` lockstep rule.
> 3. **`OrderCreatedConsumer` stub** (`Discount.Grpc/Messaging/EventHandlers/OrderCreatedConsumer.cs`) — gated on `DiscountOptions:EnableOrderCreatedConsumer=false` (default). Mirrors the `FeedbackSubmittedConsumer` Pattern 2 synthetic-claims shape (BuildBlocks contribution). Stub queries local `DbContext` for eligible coupons (avoids self-referencing gRPC); real `EvaluateDiscountRules` / gRPC redemption lands with the future ordering-scheduler plan.
> 4. **Basket-side deviation** — Basket is Marten (not EF Core), so the planned `EF migration creates the table` wording from §8.1 doesn't apply. The `BasketAppliedDiscount` child list is **embedded in the `Basket` document** (atomic parent write replaces the doc). `ApplyAllDatabaseChangesOnStartup()` covers the schema change automatically.
> 5. **`JwtForwardingInterceptor`** (`Basket.API/Auth/`) — copies inbound `Authorization: Bearer <jwt>` from `IHttpContextAccessor.HttpContext.Request.Headers` into outbound gRPC `Metadata["authorization"]`. Wired via `grpcClientBuilder.AddInterceptor<>()` in `Basket.API/Program.cs`. Discount's `AuthenticationInterceptor` authenticates via the forwarded token.
> 6. **`StoreBasketHandler` rewrite** — replaced the inline `Math.Round(...)` switch + `Math.Min(snapshots.Sum, subtotal)` clamp with `ApplyDiscountsHelper.Apply(...)`. The proto-side `DiscountType` enum (COUPON_PERCENTAGE=1 / COUPON_FIXED_AMOUNT=2) is translated to the BuildingBlocks enum (Percentage=0 / FixedAmount=1) before being passed to the helper. The legacy `DiscountAmount` field is preserved for the audit window; `EffectiveSubtotal` carries the helper's post-clamp result.
> 7. **Doc-update scope landed** — `current-architecture.md` §4.4 (Discount: `Coupon.DiscountType` column + `EnableOrderCreatedConsumer` flag), §5.2 (`OrderCreatedIntegrationEvent` row marked "Phase 8 stub wired-but-disabled"; `DiscountAppliedIntegrationEvent` SchemaVersion 1→2 row), §6 (`Coupons.DiscountType` column + Basket `EffectiveSubtotal` + `AppliedDiscountBreakdown`), §9 (`ApplyDiscountsHelper` BuildingBlocks contribution + floor-at-zero + MidpointRounding.ToEven policy).
>
> Future Discount work belongs in a new plan. The apply-surface is feature-complete for v1; multi-currency rollout + the future ordering-scheduler plan (real `EvaluateDiscountRules` / `RedeemDiscount` chain) are tracked separately.
