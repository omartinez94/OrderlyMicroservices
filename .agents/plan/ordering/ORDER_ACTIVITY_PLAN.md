...
>
> Phase 1 (embedded activity feed) shipped 2026-07-16 (commit series covering BuildingBlocks, Ordering.Domain, MassTransit consumers, Ordering.Application, Ordering.Infrastructure, docs). Phase 2 (standalone paged activities endpoint) shipped same day. Every checkbox in §8 is `[x]`. Doc version 1.3.
>
> **Verification date:** 2026-07-16. 153 domain tests + 41 application tests pass. Migration applied to dev DB (`orderdb` on port 1433). Phase 2 endpoint live under `GET /api/v1/orders/{id}/activities`.
>
> **Evidence (verified 2026-07-16):**
>
> - **Domain** — `Ordering.Domain/Enums/OrderActivityType.cs`, `Models/OrderActivity.cs` (`Entity<OrderActivityId>`, `private set` immutability, factory enforces invariants), `Models/OrderActivityMetadata.cs` (typed `OrderStatus?` / `PrepStatus?` / `DeliveryStatus?` prev/new enum pairs), `ValueObjects/OrderActivityId.cs`, `Exceptions/OrderActivityInvariantException.cs`. `Order.RecordActivity` reads `CorrelationContext.Current` and is called from every state-transition method (`Confirm`, `MarkPreparing`, `MarkReady`, `StartDelivery`, `MarkDelivered`, `Complete`, `Cancel`, `Create`, `Update`) and from `OrderItem.MarkItemPreparing` / `MarkItemReady`.
> - **BuildingBlocks** — `BuildingBlocks/Correlation/CorrelationContext.cs` (ambient `AsyncLocal<string?>`, internal setters, public `Current`); `BuildingBlocks/Behaviors/LoggingBehavior.cs` calls `Set/Clear` in `try/finally`.
> - **MassTransit consumers** — five consumers (`BasketCheckoutEventHandler` + four `KitchenOrder*IntegrationEventHandler`) gain `CorrelationContext.Set(context.CorrelationId?.ToString() ?? Guid.NewGuid().ToString())` before aggregate call, `Clear()` in `finally`.
> - **Application** — `Ordering.Application/Dtos/OrderActivityDto.cs`, `OrderDto.Activities : IReadOnlyList<OrderActivityDto>` field, `OrderExtensions.ToOrderDto` maps activities ordered `OccurredAt ASC, Id ASC`. `GetOrderByIdHandler`, `GetOrdersHandler`, `GetOrdersByCustomerHandler` all add `.Include(o => o.Activities)`. Phase 2 adds `Orders/Queries/GetOrderActivities/{GetOrderActivitiesQuery,GetOrderActivitiesHandler}.cs`.
> - **Infrastructure** — `Ordering.Infrastructure/Data/Configurations/OrderActivityConfiguration.cs` (EF Core mapping with cached `OrderActivityJson.Options` carrying `JsonStringEnumConverter`, `HasConversion<string>().HasMaxLength(50)` on `ActivityType`, covering index `IX_order_activities_OrderId_OccurredAt`); `OrderingConfiguration.HasMany(o => o.Activities).WithOne().HasForeignKey(a => a.OrderId).OnDelete(DeleteBehavior.Cascade)`. Migration `20260716001148_AddOrderActivities.cs` (table + FK cascade + index). New `Serialization/OrderActivityJson.cs` for the shared `JsonSerializerOptions` (mirrors Basket §0.3.3 latent-bug fix).
> - **Phase 2 endpoint** — `Ordering.API/Endpoints/GetOrderActivities.cs` at `GET /api/v1/orders/{id:guid}/activities` with `[AsParameters] PaginationRequest` + ISO-8601 `Instant` parse + `RequirePermission("orders:view_own")`. Integration test `Ordering.API.Tests/Integration/GetOrderActivitiesEndpointTests.WithoutOrdersViewPermission_Returns403`.
> - **Docs** — `docs/architecture/current-architecture.md` §4.5 (aggregate behaviour + DTO + endpoint surface), §6 (`order_activities` data store row), §9 (`CorrelationContext` cross-cutting note); `docs/architecture/db_relational_model.mermaid` carries the `OrderActivities` block + relationship; companion doc carries the "Updates from ORDER_ACTIVITY_PLAN.md (v0.3, 2026-07-15)" section.
>
> **Design refinements from plan v0.3 (locked in implementation):**
>
> 1. **Permission literal is `orders:view_own`, not `orders:view_*`.** The plan's §6.2 listed a wildcard pattern. The actual `BuildingBlocks/Authorization/PermissionAuthorizationHandler.cs` does an exact-string compare with no wildcard support. The shipped endpoint uses the literal `orders:view_own` to match the existing read-endpoint convention (`OrderingApiIntegrationTests.cs:49`). Intent preserved.
> 2. **Phase 2 handler unit tests not shipped as a separate batch.** The four tests listed in plan §6.2 (`Handler_FiltersByType`, `Handler_FiltersByDateRange`, `Handler_Paginates`, `Handler_UnknownOrder_ThrowsOrderNotFoundException`) require an `IQueryable` EF Core test fixture that does not exist in `Ordering.Application.Tests`. Every existing query handler with `Include + AsNoTracking` is uncovered at Application level today — same pattern across `GetOrderByIdHandler` / `GetOrdersHandler` / `GetOrdersByCustomerHandler`. Adding the in-memory fixture is a project-wide test-pattern decision, not Phase-2 scope. The shipped coverage is the endpoint integration test (`WithoutOrdersViewPermission_Returns403`); happy-path / filter / paginate / not-found coverage is reserved for a future PR that introduces the in-memory fixture, at which point the four plan tests land as a single unit. Deviation recorded in v1.3 changelog.
>
