...
- `Basket.API.Tests` (sibling project — xUnit + FluentAssertions + NSubstitute + `InMemoryDistributedCache` test double). Coverage: `BasketIdentityGuardBehaviorTests` (5 — anonymous 403 / cross-user 403 / cross-tenant 403 / happy / wrong marker), `GetBasketHandlerTests` (2 — empty-cart 200 + populated-cart), `LoggingBehaviorRedactionTests` (2 — `[PciSensitive]` redacted + non-sensitive control), `CheckoutBasketCommandHandlerTests` (3 — atomic publish-and-delete), `StoreBasketCommandValidatorTests` (3 — §0.4.10 spoofing-footgun contract), `StoreBasketHandlerTests` (3 — IsCreated True/False + empty-coupons short-circuit), `BasketIdempotencyFilterTests` (8 — UUID v4 regex + body-fingerprint HMAC + replay-on-match + 422-on-mismatch + cross-tenant + Redis-fail-closed), `CheckoutRateLimiterTests` (7 — policy + partition key + 5-permit budget + 429 with `Retry-After`), `CardSummaryDerivationTests` (3), `BasketCacheLockRegistryTests` (7 — cross-key independence + same-key serialise + caller-cancellation + dispose-cancellation + idempotent dispose + no-acquisition dispose + 32-key stress), `CachedBasketRepositoryTests` (7 — 100-concurrent coalescing on both methods + cache-hit fast-path + cross-key independence + caller-cancellation + write doesn't enter gate + invalidate passes through), `BasketExpirySweepServiceTests` (3 — `Enabled=false` short-circuit + `CancellationRequested` stops gracefully + documented defaults), `DeleteBasketHandlerTests` (3 — happy Unit return + absent-cart Unit + cancellation propagation). **Phase 3 drift item:** the "expired basket is deleted" + "live basket is untouched" assertions in §6 Phase 3 are not unit-testable — Marten's `IMartenQueryable<T>` chain is not mockable via NSubstitute. These are the Testcontainers + Postgres integration tests in Phase 5.
