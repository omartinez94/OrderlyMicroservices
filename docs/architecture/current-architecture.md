...
> match.
>
> **Last touched (Phase 3, 2026-07-25):** Basket §4.3 — removed the
> Phase 1 `[DEPRECATED]` shim suite (`/api/v1/baskets/{userId}/{restaurantId}`
> GET/PUT/DELETE + `/api/v1/baskets/checkout`); DELETE returns
> 204 No Content (idempotent); `Cache-Control: no-store` on every
> response; single-flight cache stampede protection via
> `IBasketCacheLockRegistry`; `BasketExpirySweepService`
> `BackgroundService` deletes carts whose `ExpiresAt` is in the past.
> The legacy URL returns 404 from the router — no `410 Gone` header.
...
|---|---|---|---|
| GET | `/api/v1/cart` | `orders:view_own` | **Token-bound**. `(UserId, RestaurantId)` come from JWT. Returns 200 + empty cart body when no cart exists (never 404). `Cache-Control: no-store` on every response (cart contains PII). |
...
- **Tracing / metrics:** no OpenTelemetry / Application Insights integration in code. **Phase 3 follows:** the `CachedBasketRepository` records a `System.Diagnostics.Metrics` counter (`basket.cache.requests` with `result=hit|miss`) on every short-circuit (`GetStringAsync` returns non-empty) and miss path, respectively. Exposed via `IMeterFactory` once the host gains an OpenTelemetry exporter in Phase 4 — until then the counter is wired but not scraped.
