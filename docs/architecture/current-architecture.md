...
- **Interceptors.** `BuildingBlocks.Entities.Interceptors.AuditableEntityInterceptor` is registered in every service. **`DispatchDomainEventsInterceptor`** (in-process domain-event dispatch) is registered in Ordering, Kitchen, and **Catalog (Phase 3)**. `OrderingOutboxPublisher` (`Ordering.Infrastructure`) and `KitchenOutboxPublisher` (`Kitchen.API`) intercept `SaveChangesAsync` to write `outbox_messages` rows inside the same EF Core transaction as the aggregate mutation; the matching dispatcher hosted service relays the rows to `IPublishEndpoint`. **Catalog's Phase 2 implementation skips the publisher-as-interceptor pattern** — there is no `IDomainEvent` infrastructure in Catalog today (Phase 2). Phase 3 introduces `IDomainEvent` / `IAggregate` / `DispatchDomainEventsInterceptor` (Catalog-local, mirror of Kitchen), wired alongside `AuditableEntityInterceptor` via DI-resolved `ISaveChangesInterceptor` resolution (Ordering-style: `options.AddInterceptors(sp.GetServices<ISaveChangesInterceptor>())`). Aggregate bases: `Ingredient : AuditableAggregate<int>`, `IngredientAlternative : Aggregate<int>`, `MenuItemIngredient : Aggregate<int>`. The engine's domain events are `IngredientChangedDomainEvent`, `IngredientAlternativeChangedDomainEvent`, `MenuItemIngredientChangedDomainEvent` (each a sealed `record` with `init`-only `EventId` / `OccurredOn` per Kitchen's pattern; full type qualification on the nested `ChangeKind` enum avoids forward-reference errors in C# record primary constructors). `Catalog.API/Application/EventHandlers/Domain/IngredientAvailabilityChangedDomainEventHandler.cs` is a single `INotificationHandler<IDomainEvent>` (switch on event type) that consumes every event the engine's surface raises and routes to the right input-set loader.
