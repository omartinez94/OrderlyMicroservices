...
> match.
>
> **Last touched (Phase 4, 2026-07-25):** Basket §4.3 — gRPC
> resilience pipeline (Polly v8: 3× retry, 5-failure circuit
> breaker, 3s attempt / 8s total timeout); OpenTelemetry tracing
> + metrics (ASP.NET Core + HttpClient + Marten + MassTransit +
> Npgsql sources) with OTLP export; `/live` (process-alive) +
> `/ready` (Postgres + Redis + RabbitMQ) health split;
> `X-Correlation-Id` flows from `LoggingBehavior` scope into the
> OTel `Activity` bag via `CorrelationIdActivityMiddleware`;
> `ETag` (SHA-256 of basket JSON) + `Last-Modified` (basket
> `LastModifiedAt`) on `GET /api/v1/cart` with 304 Not Modified
> conditional-GET support; Swashbuckle + OpenAPI 2.7.5 wired
> (`/swagger/v1/swagger.json` in Development);
> `/api/v1/admin/carts/{GET,PUT,DELETE}` requires the new
> `orders:admin` permission (seeded in Identity.API
> `DataSeeder.cs`); `BasketAuditLogEntry` Marten document +
> `IBasketAuditLog` interface for cross-account mutation audit.
> Phase 4 plan v1.8 (basket service).
...
**Two-tier design.** Marten is the durable store; Redis is the cache wrapper applied via `services.Decorate<IBasketRepository, CachedBasketRepository>()`. Cache key is `basket:{userId}:{restaurantId}`; on miss the basket is reloaded from Marten and re-cached for 30 min. Empty carts are cached too (closing the loop hole where concurrent "no cart yet" reads each triggered a Marten query).

**Single-flight cache stampede protection.** `CachedBasketRepository.GetOrLoadWithSingleFlightAsync` uses a per-`(userId, restaurantId)` `SemaphoreSlim` (singleton lifetime via `IBasketCacheLockRegistry`) to coalesce N concurrent cache-miss reads on the same key into ONE inner-repository query. The double-checked locking pattern re-checks the cache inside the gate so the 2nd-and-later holders see the warmed cache and skip the inner round-trip. The registry is a singleton with idempotent `IAsyncDisposable` — the host's `IHostApplicationLifetime.ApplicationStopping` callback disposes it (cancels the registry-wide `CancellationTokenSource` so pending waiters raise `OperationCanceledException` promptly; the host's `IHostOptions.ShutdownTimeout` (default 30s) bounds the drain). Semaphore disposal is best-effort; ObjectDisposedException from a holder releasing mid-shutdown is caught silently. The Kafka-style "first holder writes cache before releasing" pattern is essential — releasing-then-writing would let the 2nd holder re-run the inner before the 1st holder's write completes, defeating the coalescing.
...
public class Basket : ITenantEntity
{
    ...
    public Instant ExpiresAt { get; set; }     // enforced by BasketExpirySweepService (every 5 min by default)
}
...
|---|---|---|---|
| GET | `/api/v1/cart` | `orders:view_own` | **Token-bound**. `(UserId, RestaurantId)` come from JWT. Returns 200 + empty cart body when no cart exists (never 404). `Cache-Control: no-store` on every response (cart contains PII). **`ETag` (SHA-256 of basket JSON) + `Last-Modified` (`basket.LastModifiedAt`)** headers drive conditional GET — `If-None-Match` / `If-Modified-Since` matching returns **304 Not Modified** with no body (Phase 4). |
| PUT | `/api/v1/cart` | `orders:create` | **Token-bound**. Body `UserId`/`RestaurantId` MUST be `Guid.Empty` (§0.4.10 spoofing-footgun validator rejects any other value with **422**); the endpoint overwrites them with the JWT-derived identity before constructing the command so the identity guard sees matching values. Returns **201 Created** + `Location: /api/v1/cart` on the first PUT (new cart) and **200 OK** on every subsequent PUT (idempotent upsert). `Cache-Control: no-store`. |
| DELETE | `/api/v1/cart` | `orders:create` | **Token-bound**. Idempotent — returns **204 No Content** (no body) regardless of whether the cart existed. `Cache-Control: no-store`. |
| POST | `/api/v1/cart/checkout` | `orders:create` | **Token-bound**. Body `UserId`/`RestaurantId` enforced by the identity guard. `Cache-Control: no-store`. |
| GET | `/api/v1/admin/carts` | `orders:admin` | **Cross-account admin surface (Phase 4)**. Paged list of carts in the active tenant. `RestaurantSupportAgent` tooling. Audit log row written on every mutation. |
| PUT | `/api/v1/admin/carts/{userId}` | `orders:admin` | **Cross-account admin upsert (Phase 4)**. Body `Models.Basket`; URL `userId` + JWT `restaurantId` are authoritative (spoofing-footgun guard). Returns 201 + `Location: /api/v1/cart` (new) or 200 (update). |
| DELETE | `/api/v1/admin/carts/{userId}` | `orders:admin` | **Cross-account admin delete (Phase 4)**. Idempotent — returns 204 regardless. |
| GET | `/health` | public | (legacy — see also `/live` + `/ready` below) |
| GET | `/live` | public | **Liveness probe (Phase 4)**. Process-alive only — no external dependencies. |
| GET | `/ready` | public | **Readiness probe (Phase 4)**. Checks every backing store tagged `ready` (Postgres + Redis + RabbitMQ). |
| GET | `/swagger/v1/swagger.json` | public (dev only) | **OpenAPI spec (Phase 4)**. Generated by Swashbuckle 10.2.3 + Microsoft.OpenApi 2.7.5. Mounted under `/swagger` in Development only. |

The Phase 1 `[DEPRECATED]` `/api/v1/baskets/{userId}/{restaurantId}` shim suite (GET / PUT / DELETE / POST-checkout) was removed end of Phase 3. The router returns **404 Not Found** for any caller that still hits the legacy URL — the migration signal is the 404, not a `410 Gone` header. External callers must migrate to the token-bound `/api/v1/cart` endpoints.
...
**TTL semantics.** Redis side: 30-minute absolute TTL on cache reads/writes (empty carts included — Phase 3 closes the loop hole where concurrent "no cart yet" reads each triggered a Marten query). Marten side: `Basket.ExpiresAt` is enforced by `BasketExpirySweepService` (`Basket.API/Services/BasketExpirySweepService.cs`), a `BackgroundService` that polls every `Basket:ExpirySweep:Interval` (default 5 min, bound via `IOptions<BasketOptions>` with `ValidateOnStart()`). Per tick it queries `mt_doc_basket` for `(UserId, RestaurantId)` pairs with `ExpiresAt < clock.GetCurrentInstant()`, takes up to `Basket:ExpirySweep:BatchSize` rows (default 1 000), and deletes each via `IDocumentSession.Delete<>` followed by a single `SaveChangesAsync`. **No `BasketCheckoutEvent` is published** — the sweep is pure housekeeping; the cart is being discarded, not promoted to an order. The dispatcher is multi-replica-safe via Marten's `mt_version` optimistic-concurrency fence: a concurrent delete on a peer replica raises `ConcurrencyException` on the second `SaveChangesAsync`, which is logged and the loop continues. Cross-tenant by design — the sweep service is the one legitimate caller that bypasses `IBasketRepository.AssertTenant` via a direct `IDocumentStore.LightweightSession()` per tick. Disposal is `IAsyncDisposable` so the `using` block waits for the background task to exit (sync `Dispose` would cancel the `CancellationTokenSource` but not wait — the test host would hang on shutdown).
...
6. Basket.API migrates and seeds the per-tenant `basketdb` databases on first start (per-tenant database creation via `CreateDatabasesForTenants`). The `CheckoutBasketOutboxDispatcher` and `BasketExpirySweepService` `BackgroundService`s start alongside the API; both honour the `Basket:ExpirySweep:Enabled` and `Outbox:Enabled` flags (set to `false` in tests to keep the host quiet). The cache-lock registry's `IAsyncDisposable` is wired into `IHostApplicationLifetime.ApplicationStopping` so the registry disposes cleanly on shutdown.
7. Ordering migrates with 30-attempt retry and seeds four customers, two menu items, four orders, four bills. The `AddOutboxMessages` migration runs alongside the existing ones; no extra command is required. The `TypedOrderItemCustomizationsJsonb` migration is empty at the SQL level (only the .NET property type changes; the on-disk column stays `nvarchar(max)` jsonb) and lands automatically with the existing migration set.
7a. Kitchen.API migrates the `kitchendb` schema (3 tables + `outbox_messages`). Both services start their outbox dispatcher hosted services alongside the API; tests in either project flip `Outbox:Enabled=false` to skip the relay loop.
8. Discount uses `EF Core Migrations` and runs `Database.MigrateAsync()` on startup; seed data is in `OnModelCreating`. Coupon is the single Discount entity (no Catalog-side writers, no backfill). `db_relational_model.mermaid` and `db_relational_model.md` updated to drop the Catalog-side Coupon block and the `Restaurants ||--o{ Coupons` relationship row; the `Coupon` entry is removed from the `AuditableEntity<TId>` user list in the companion doc.
...
- `Basket.API.Tests` (sibling project — xUnit + FluentAssertions + NSubstitute + Verify + Testcontainers + `WebApplicationFactory<Program>`). 138 / 138 tests passing (124 unit + 14 integration + 0 smoke after the Verify snapshot consolidation). The WAF (`BasketWebApplicationFactory` + its `BasketExpirySweepWebApplicationFactory` subclass) spins up **Postgres 16 + Redis 7 + RabbitMQ 3-management** in Testcontainers per test run; env vars (`ConnectionStrings__BasketDB`, `ConnectionStrings__Redis`, `MessageBroker__*`, `GrpcSettings__DiscountUrl`) are pre-loaded BEFORE `CreateClient()` because the API host reads connection strings eagerly in `Program.cs:61` and `Program.cs:127-129`. `TestAuthHandler` replaces the JWT bearer scheme; missing `X-Test-User` returns `NoResult()` → 401. `appsettings.Test.json` overlays the test config (`Outbox:Enabled=false`, `Basket:ExpirySweep:Enabled=false`, `OpenTelemetry:Enabled=false`, the `Basket:Idempotency:SecretHex` dev placeholder). `xunit.runner.json` (`parallelizeTestCollections=false` + `parallelizeAssembly=false`) closes the cross-collection race on the static `BasketCacheLockRegistry`. Coverage: `BasketIdentityGuardBehaviorTests` (5), `GetBasketHandlerTests` (2), `LoggingBehaviorRedactionTests` (2), `CheckoutBasketCommandHandlerTests` (3 — atomic publish-and-delete), `StoreBasketCommandValidatorTests` (3 — §0.4.10 spoofing-footgun), `StoreBasketHandlerTests` (13 — IsCreated True/False + empty-coupons short-circuit + real-discount path), `BasketIdempotencyFilterTests` (10 — UUID v4 regex + body-fingerprint HMAC + replay-on-match + 422-on-mismatch + cross-tenant + Redis-fail-closed + `StubOkResult` IResult-capture path), `CheckoutRateLimiterTests` (3 — policy + partition key + 5-permit budget + 429 with `Retry-After`), `CardSummaryDerivationTests` (3), `BasketCacheLockRegistryTests` (7), `CachedBasketRepositoryTests` (7), `BasketExpirySweepServiceTests` (3 — `Enabled=false` short-circuit + `CancellationRequested` stops gracefully + documented defaults), `DeleteBasketHandlerTests` (3), `DiscountGrpcResiliencePipelineTests` (1), `OtelOptionsTests` (3), `ETagTests` (6), `BasketExpirySweepTests` (4 — expired-deleted + live-untouched + mixed-batch + empty-store via Testcontainers + Postgres fan-out; `BasketExpirySweepService.SweepOnceAsync` driven directly via `IEnumerable<IHostedService>.OfType<>()`), `WafSmokeTests` (2 — `/live` 200 + auth-header round-trip), `GetCartEndpointTests` (4 — empty + populated + anonymous 401 + ETag 304), `UpsertCartEndpointTests` (6 — new + existing + spoofed-UserId/RestaurantId overwrite + anonymous + validation 400; uses `NodaTimeJson.CreateContent` to mirror the host's `ConfigureForNodaTime` JSON config), `DeleteCartEndpointTests` (3 — existing 204 + absent 204 + anonymous 401), `CheckoutCartEndpointTests` (8 — happy + replay-200 + different-body-422 + no-cart-404 + empty-400 + missing/malformed-idempotency-key-400 + anonymous 401), `OpenApiGenerationTests` (1 — every `MapBasketGroup` endpoint enumerated by `AddEndpointsApiExplorer` via `ISwaggerProvider.GetSwagger("v1")` from DI; regression detector for any future endpoint registered outside the canonical group), `BasketSnapshotsTests` (1 — Verify snapshot of the full OpenAPI document, first snapshot in the repo; the `.verified.txt` co-located with the test class is the canonical lock for the API surface).

---

### Test bootstrap snippet
- `dotnet test orderly-microservices/Services/Basket/Basket.API.Tests` runs all 138 Basket tests. Integration tests spin up Testcontainers-managed Postgres + Redis + RabbitMQ on the host's Docker daemon; the first run may take longer while images are pulled. No manually provisioned dependent services are required. The CI workflow (`orderly-microservices/.github/workflows/basket-tests.yml`) runs the same command on `ubuntu-latest` and separates the unit (filter excludes `Integration`) and integration (filter includes `Integration`) steps for cache locality; see `.agents/plan/basket/BASKET_SERVICE_PLAN.md` §6 Phase 5 for the phased delivery log.
- The Basket OpenAPI surface is committed at `orderly-microservices/docs/api/basket-api-v1.json` (regenerate via `orderly-microservices/scripts/generate-basket-openapi.ps1` — requires the Basket host running in `ASPNETCORE_ENVIRONMENT=Development` so `app.UseSwagger()` is reachable). The `OpenApiGenerationTests.AllEndpointsDocumented` integration test guards against any endpoint registered without `AddEndpointsApiExplorer` discovery; the `BasketSnapshotsTests.VerifyAllEndpoints` Verify snapshot locks the full document shape. First such file in the repo — establishes the convention that future service OpenAPI mirrors follow.
...
- **Tracing / metrics:** **Phase 4: full OpenTelemetry pipeline.** `services.AddOpenTelemetry()` in `Program.cs` wires `WithTracing` (ASP.NET Core + HttpClient + Marten `ActivitySource "Marten"` + MassTransit `ActivitySource "MassTransit"` + Npgsql) and `WithMetrics` (ASP.NET Core + Runtime). The OTLP exporter is conditional on `OpenTelemetry:Enabled` (default true). The `X-Correlation-Id` inbound header is bridged into the OTel `Activity` tag bag via `CorrelationIdActivityMiddleware` (mounted first in the pipeline), so every emitted span carries the same id as the `LoggingBehavior` `BeginScope` log line. The CachedBasketRepository records a `System.Diagnostics.Metrics` counter (`basket.cache.requests` with `result=hit|miss`) on every short-circuit and miss path. Exposed via OTLP — bring up an OpenTelemetry Collector in Phase 5 alongside the Testcontainers harness.
- **Test-only suppression.** `BasketWebApplicationFactory` sets `OpenTelemetry:Enabled=false` (via `appsettings.Test.json` + the in-memory config override at `BasketWebApplicationFactory.cs:96`), which suppresses the OTLP exporter. Test runs do NOT attempt to reach an external collector; the in-process `System.Diagnostics.Activity` is still emitted and is available via `Activity.Current` for any test that wants to assert on spans. No in-memory exporter (`AddInMemoryExporter` / `OpenTelemetry.Exporter.InMemory`) is wired today — Phase 5.x follow-up if span-assertion tests are added.
