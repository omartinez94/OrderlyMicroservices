...
/// <summary>
/// Replaces the production JWT bearer handler in the integration test
/// host. Reads <c>X-Test-User</c> + <c>X-Test-Permissions</c> from the
/// request and turns them into a <see cref="ClaimsPrincipal"/>. Requests
/// without those headers fall through to the regular pipeline and get
/// 401 — letting integration tests assert the unauthenticated path
/// without juggling JWT tokens.
/// </summary>
